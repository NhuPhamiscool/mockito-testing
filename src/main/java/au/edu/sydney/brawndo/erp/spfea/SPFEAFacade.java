...
public class SPFEAFacade {
    ...
    public List<Integer> getAllOrders() {
        ...
        // List<Order> orders = uow.getOrders(token);
    ...
    public Integer createOrder(int customerID, LocalDateTime date, boolean isBusiness, boolean isSubscription, int discountType, int discountThreshold, int discountRateRaw, int numShipments) {
        ...
        // int id = uow.getNextOrderID();
        int id = TestDatabase.getInstance().getNextOrderID();

        if (isSubscription) {
            if (1 == discountType) { // 1 is flat rate
                    if (isBusiness) {
                        order = new BusinessOrder(id, customerID, date, new SubscriptionOrder(new FlatRateDiscount(discountRate), numShipments));
                        //  new NewOrderImplSubscription(id, date, customerID, discountRate, numShipments);
                    } else {
                        order = new PersonalOrder(id, customerID, date, new SubscriptionOrder(new FlatRateDiscount(discountRate), numShipments));
                        // new Order66Subscription(id, date, discountRate, customerID, numShipments);
                    }
                } else if (2 == discountType) { // 2 is bulk discount
                    if (isBusiness) {
                        order = new BusinessOrder(id, customerID, date, new SubscriptionOrder(new BulkDiscount(discountRate, discountThreshold), numShipments));
                        // new BusinessBulkDiscountSubscription(id, customerID, date, discountThreshold, discountRate, numShipments);
                    } else {
                        order = new PersonalOrder(id, customerID, date, new SubscriptionOrder(new BulkDiscount(discountRate, discountThreshold), numShipments));
                        // new FirstOrderSubscription(id, date, discountRate, discountThreshold, customerID, numShipments);
                    }
            } else {return null;}
        } else {
            if (1 == discountType) {
                if (isBusiness) {
                    order = new BusinessOrder(id, customerID, date, new OneOffOrder(new FlatRateDiscount(discountRate)));
                    // new NewOrderImpl(id, date, customerID, discountRate);
                } else {
                    order = new PersonalOrder(id, customerID, date, new OneOffOrder(new FlatRateDiscount(discountRate)));
                    // new Order66(id, date, discountRate, customerID);
                }
            } else if (2 == discountType) {
                if (isBusiness) {
                    order = new BusinessOrder(id, customerID, date, new OneOffOrder(new BulkDiscount(discountRate, discountThreshold)));
                    // new BusinessBulkDiscountOrder(id, customerID, date, discountThreshold, discountRate);
                } else {
                    order = new PersonalOrder(id, customerID, date, new OneOffOrder(new BulkDiscount(discountRate, discountThreshold)));
                    // new FirstOrder(id, date, discountRate, discountThreshold, customerID);
                }
            } else {return null;}
        }

        // TestDatabase.getInstance().saveOrder(token, order);
        uow.addOrder(token, order);
    ...
    public boolean removeOrder(int id) {
        ...
        // return uow.removeOrder(token, id);
    }
    ...
    public boolean finaliseOrder(int orderID, List<String> contactPriority) {
        ...
        // Order order = uow.getOrder(token, orderID);
